...
export const translateVideo = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (req.file) {
            ...
            translatingData.video_url = `${process.env.API_URL}/api/translate/file/${req.file.originalname}`;
        ...
        const response = await heygenApi.post("/v2/video_translate", translatingData);
        ...
        res.status(200).json({ message: "Translation started", data: response.data });
    } catch (error: any) {
        console.error("Error in translation:", error.message || error.response?.data);
        res.status(500).json({
