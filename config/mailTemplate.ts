...
export function individualServiceUserEmail(userName: string, service: any) {
    return `<!DOCTYPE html>
              <html>
              <head>
                  ${style}
  ...
  export function individualServicePaymentSuccessAdmin(
    userName: string,
    userEmail: string,
    service: any
  ) {
...
export function sendSubscriberInvites(   
    fullName: string,   
    ebookLink: string,
    inviteId: string | null = null
) {
    const ebookSection = ebookLink
    ? `<p>As a special welcome, we're offering you a free eBook: <a href="${ebookLink}?subscriberInvite=${inviteId}">View eBook here</a></p>`
    : "";
    return `<!DOCTYPE html>
            <html>
            <head>
